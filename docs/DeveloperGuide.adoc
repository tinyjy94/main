= MoviePlanner Level 4 - Developer Guide
:toc:
:toc-title:
...
* can save the Movie Planner data in xml format and read it back.
...
As you can see from the diagram, `UndoableCommand` adds an extra layer between the abstract `Command` class and concrete commands that can be undone, such as the `DeleteCommand`. Note that extra tasks need to be done when executing a command in an _undoable_ way, such as saving the state of the movie planner before execution. `UndoableCommand` contains the high-level algorithm for those extra tasks while the child classes implements the details of how to execute the specific command. Note that this technique of putting the high-level algorithm in the parent class and lower-level steps of the algorithm in child classes is also known as the https://www.tutorialspoint.com/design_pattern/template_pattern.htm[template pattern].
...
The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th cinema in the movie planner. The current state of the movie planner is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command).
...
We will pop the most recent command out of the `undoStack` and push it back to the `redoStack`. We will restore the movie planner to the state before the `add` command executed.
...
The redo does the exact opposite (pops from `redoStack`, push to `undoStack`, and restores the movie planner to the state after the command is executed).
...
A project often depends on third-party libraries. For example, Movie Planner depends on the http://wiki.fasterxml.com/JacksonHome[Jackson library] for XML parsing. Managing these _dependencies_ can be automated using Gradle. For example, Gradle can download the dependencies automatically, which is better than these alternatives. +
