...
Figure 19 below shows how the components interact for the scenario where the user issues the command `encrypt pw/pass`. Figure 20 
shows how the `EventsCenter` reacts to that event, which eventually results in the updates being saved to the hard disk.
