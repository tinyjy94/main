...
. Creates an `AddScreeningCommand` object that takes in the parameters. In `AddScreeningCommand`, `isValidScreening()` ensures a couple of properties:
... The movie index provided is valid.
... The cinema index provided is valid.
... The theater number provided is valid.
... The screening can fit into existing theater schedule and the start date of screening is not before the release date of the movie.
...
[start = 4]
. Once a screening is validated, `AddScreeningCommand` will call the constructor of `Screening` to create a screening object.

. `AddScreeningCommand` will call `model` to update the cinema with the new screening object.
