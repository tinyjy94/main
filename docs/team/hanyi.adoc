...
MoVie Planner (MVP) - is a desktop movie planner built to streamlining a theater manager's job by managing schedules of movies in different cinemas. The user interacts with it using a CLI, and it has a GUI created with JavaFX. It is written in Java, and has about 20 kLoc.
